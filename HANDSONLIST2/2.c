...
void main()
{
    ...
    // RLIMIT_NOFILE -> Maximum number of file descriptors that can be opened by this process
    status = getrlimit(RLIMIT_NOFILE, &resourceLimits);
    if (status == -1)
        perror("Error while getting RLIMIT_NOFILE!");
    else
        print("Max file descriptor", resourceLimits.rlim_cur, resourceLimits.rlim_max, "");
    ...
    if (status == -1)
        ...
        print("Amount of CPU time for a real-time scheduled process", resourceLimits.rlim_cur, resourceLimits.rlim_max, "ms");
    ...
    status = getrlimit(RLIMIT_STACK, &resourceLimits);  // Corrected this line
    if (status == -1)
